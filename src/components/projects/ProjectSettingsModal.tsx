  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-secondary/50">
          <span className="px-2.5 py-0.5 rounded-full text-xs font-medium text-white opacity-60" style={{ background: newColor }}>New</span>
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Add new stage..."
            className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground"
            onKeyDown={e => { if (e.key === 'Enter' && newName.trim()) addMut.mutate(); }} />
          <div className="flex gap-1">{PRESET_COLORS.slice(0, 5).map(c => (
            <button key={c} onClick={() => setNewColor(c)} className={`w-4 h-4 rounded-full border-2 ${newColor === c ? 'border-foreground' : 'border-transparent'}`} style={{ background: c }} />
          ))}</div>
          <button onClick={() => addMut.mutate()} disabled={!newName.trim()} className="p-1 rounded bg-primary text-primary-foreground disabled:opacity-50">
            <Plus className="h-3.5 w-3.5" />
          </button>
        </div>
      </div>
