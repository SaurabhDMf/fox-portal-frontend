...
import {
  ...
  FileSpreadsheet, Archive, File, Loader2,
} from 'lucide-react';
...
function fileIconProps(mime: string) {
  if (mime?.startsWith('image/'))                        return { Icon: Image,         color: 'text-blue-500',  bg: 'bg-blue-500/10'  };
  if (mime === 'application/pdf')                        return { Icon: FileText,       color: 'text-red-500',   bg: 'bg-red-500/10'   };
  if (mime?.includes('spreadsheet') || mime?.includes('excel') || mime?.includes('csv'))
                                                         return { Icon: FileSpreadsheet, color: 'text-green-500', bg: 'bg-green-500/10' };
  if (mime?.includes('zip'))                             return { Icon: Archive,        color: 'text-amber-500', bg: 'bg-amber-500/10' };
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-8 text-center text-sm text-muted-foreground">No documents uploaded yet</div>
      ) : (
