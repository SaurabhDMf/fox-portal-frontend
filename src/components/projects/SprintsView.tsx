...
export default function SprintsView({ projectId, onTaskClick, onCreateTask }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {sprints.map(sprint => {
          const total = (sprint as any).total_tasks ?? sprint.task_count ?? 0;
          const done = (sprint as any).done_tasks ?? sprint.done_count ?? 0;
          const progress = total ? Math.round((done / total) * 100) : 0;

          return (
            <div key={sprint.id} className="glass-card p-4 space-y-3 cursor-pointer hover:ring-1 hover:ring-primary/30 transition-all" onClick={() => setSelectedSprint(sprint)}>
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={() => setDeleteSprintId(sprint.id)} className="p-1.5 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
              ...
              {total > 0 && (
                <div className="space-y-1">
                  <div className="flex justify-between text-xs"><span className="text-muted-foreground">Progress</span><span>{done}/{total} tasks ({progress}%)</span></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md p-6 space-y-4 animate-slide-up">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs text-muted-foreground">Start</label><input type="date" value={form.start_date} onChange={e => setForm(f => ({ ...f, start_date: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-sm focus:outline-none" /></div>
              <div><label className="text-xs text-muted-foreground">End</label><input type="date" value={form.end_date} onChange={e => setForm(f => ({ ...f, end_date: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-sm focus:outline-none" /></div>
            </div>
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => createMut.mutate(form)} disabled={!form.name || createMut.isPending} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50">
      ...
      {showComplete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-sm p-6 space-y-4 animate-slide-up">
            ...
            <select value={moveIncompleteTo} onChange={e => setMoveIncompleteTo(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-sm focus:outline-none">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => completeMut.mutate(showComplete)} disabled={completeMut.isPending} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50">
      ...
      {deleteSprintId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-sm p-6 space-y-4 animate-slide-up">
            ...
            <p className="text-sm text-muted-foreground">Are you sure? Modules will be moved back to the backlog.</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteSprintId(null)} className="px-4 py-2 rounded-lg text-sm text-muted-foreground hover:bg-secondary">Cancel</button>
              <button onClick={() => deleteMut.mutate(deleteSprintId)} disabled={deleteMut.isPending} className="px-4 py-2 rounded-lg bg-destructive text-destructive-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50">
