...
export default function CPCreateDMModal({ onClose, onCreated }: Props) {
  const qc = useQueryClient();
  const [search, setSearch] = useState('');

  const { data: contacts = [], isLoading } = useQuery({
    queryKey: ['client-contacts'],
    queryFn: () => api.get('/client/contacts').then(r => {
      const d = r.data;
  ...
  const filtered = (contacts as any[]).filter(u =>
  ...
  const mut = useMutation({
    ...
    onError: (e: any) => toast.error(e.response?.data?.message || 'Failed to start conversation'),
  });
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-0.5 border border-border rounded-lg max-h-64">
          ...
          {!isLoading && filtered.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">No team contacts found</p>
          )}
          {filtered.map((u: any) => (
            <button
              key={u.id}
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-muted-foreground truncate">{u.job_title || u.role || u.email}</div>
              </div>
            </button>
          ))}
        </div>
