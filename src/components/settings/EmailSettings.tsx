...
export default function EmailSettings() {
  ...
  const set = <K extends keyof SmtpForm>(key: K, val: SmtpForm[K]) =>
    setForm(f => ({ ...f, [key]: val }));
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      ...
      if (form.smtp_pass.trim().length > 0) {
  ...
  return (
    <div className="space-y-4">
      <div className="glass-card p-6 space-y-5">
        <div>
          <h2 className="text-sm font-semibold">Email (SMTP) Settings</h2>
      ...
      <div className="glass-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-end">
          <div>
            ...
            <input
              type="email"
              placeholder={form.smtp_user || 'recipient@example.com'}
              value={testTo}
              onChange={e => setTestTo(e.target.value)}
              className={inputCls}
            />
            <p className="text-[11px] text-muted-foreground mt-1">
              Leave blank to send to the SMTP username.
            </p>
          </div>
          <button
            type="button"
            onClick={() => testMut.mutate()}
            disabled={testMut.isPending || !form.smtp_host.trim()}
