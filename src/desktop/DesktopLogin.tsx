...
export default function DesktopLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuth(res.data);
      toast.success(`Welcome back, ${res.data.user?.full_name || 'User'}!`);
      navigate('/app');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      <div className="w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Foxportal Desktop</h1>
          <p className="text-muted-foreground text-xs mt-2">Sign in with your Foxportal account</p>
        </div>
