...
export default function Invoicing() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/invoices', { params: { status: tab === 'All' ? undefined : tab, search } }).then(r => r.data),
  });
          ...
          <tbody>
            ...
            (Array.isArray(invoices) ? invoices : []).map((inv: any) => (
              <tr key={inv.id} className="border-b border-border/50 hover:bg-secondary/50 transition-colors">
                ...
                <td className="p-4">
                  <span className={inv.status === 'Paid' ? 'badge-success' : inv.status === 'Overdue' ? 'badge-danger' : inv.status === 'Sent' ? 'badge-info' : 'badge-neutral'}>{inv.status}</span>
                </td>
                <td className="p-4">
                  {inv.status === 'Draft' && (
