...
export default function SAUsers() {
  ...
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => api.get('/sa/users', { params: search ? { search } : {} }).then(r => r.data?.users ?? r.data ?? []),
    retry: 1,
  });
    ...
    <div className="page-container">
      ...
      {error ? (
        <div className="glass-card p-8 text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">{(error as any)?.response?.data?.error || (error as Error).message}</p>
        </div>
      ) : (
